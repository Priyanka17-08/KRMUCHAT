...
import { Send, Bot, User, Loader2, FileText, Search, ArrowLeft } from 'lucide-react';
...
export default function FacultyAssistantPage() {
  ...
  const [messages, setMessages] = useState<Array<{ role: 'user' | 'assistant', content: string }>>([
    { role: 'assistant', content: `Hello ${user?.name.split(' ')[0]}! Ask me anything about faculty members, departments, office hours, or research areas.` }
  ]);
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] p-4 md:p-6">
      ...
      <div className="flex items-center mb-4">
        ...
        <h1 className="text-xl font-bold text-white">Faculty Directory & Assistant</h1>
      </div>
      ...
      <section className="w-full lg:w-1/2 flex flex-col bg-slate-900/40 border border-slate-800/50 rounded-3xl backdrop-blur-sm overflow-hidden">
        <div className="p-6 border-b border-slate-800/50">
          <h2 className="text-lg font-bold text-slate-200 uppercase tracking-wider text-xs mb-4">Faculty Directory</h2>
          <div className="relative">
            <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" />
            <input type="text" placeholder="Search faculty by name or department..." className="w-full bg-slate-800/50 border border-slate-700 rounded-full py-2 pl-10 pr-4 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500/50" />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {facultyMembers.map((faculty) => (
            <div key={faculty.id} className="bg-white/5 border border-white/5 rounded-2xl p-4 hover:bg-white/10 transition-colors">
              <h3 className="text-sm font-semibold text-slate-200">{faculty.name}</h3>
              <p className="text-xs text-blue-400 mb-2">{faculty.designation} - {faculty.department}</p>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3 max-w-2xl mx-auto w-full">
            <div className="bg-slate-800/80 border border-slate-700/50 rounded-2xl p-2 flex items-center gap-2">
              <input
                ...
                placeholder="Ask about faculty, office hours, or departments..."
